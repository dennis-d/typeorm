...
export class SqlServerQueryRunner extends BaseQueryRunner implements QueryRunner {
    ...
    protected addTemporalColumnsSql(table:Table): string {
        let sql: string = "" ;
        if (table.temporal) {
            sql += ` , "${table.temporal.sysStartTimeColumnName}" datetime2 (2) GENERATED ALWAYS AS ROW START NOT NULL`;
            ...
            sql += `, "${table.temporal.sysEndTimeColumnName}" datetime2 (2) GENERATED ALWAYS AS ROW END NOT NULL`;
